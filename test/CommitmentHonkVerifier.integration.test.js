...
// circuits/commitment_update — not MockVerifier. See SolvencyHonkVerifier.integration.test.js
// for the full explanation of the calldata mapping (publicInputs = the circuit's own 5
// declared public inputs; proof = bb's unmodified proof blob).
describe("CommitmentHonkVerifier (real proof, real verifier)", function () {
  ...
  before(async function () {
    ...
    verifier = await deployHonkVerifier("CommitmentHonkVerifier", "CommitmentHonkVerifier");
  });

  it("verifies a real proof of a fresh deposit (old_commitment == 0, delta == 100)", async function () {
  ...
  it("rejects the same proof against a tampered public input", async function () {
    ...
    tampered[tampered.length - 1] ^= 0xff; // flip a bit in the last public input (assetId)
