...
//
// The projection mirrors AssetRegistry.currentSupplyIndexRay exactly, which means it has to
// take the RAY-scaled rate, not the basis-point one. Driving it off `supplyRateBps` looked
// equivalent and was not: that view floors to zero for any rate under a basis point, which
// is most of a young market's life, so the projection quietly became a no-op on precisely
// the markets that need it.
const RAY = 10n ** 18n;
const SECONDS_PER_YEAR = 31_536_000n;

// How long a prepared deposit stays submittable. Generous on purpose: the cost of
// overshooting is a dust rounding against the depositor (at these rates, far below one unit
// of the token over half an hour), while the cost of undershooting is a reverted
// transaction and a confused user.
export const SUPPLY_INDEX_PROJECTION_SECONDS = 1_800n;

export function projectSupplyIndexRay(indexRay: bigint, supplyRateRay: bigint, elapsedSeconds = SUPPLY_INDEX_PROJECTION_SECONDS): bigint {
  if (supplyRateRay === 0n) return indexRay; // an idle asset's index doesn't move at all
  return indexRay + (indexRay * supplyRateRay * elapsedSeconds) / (RAY * SECONDS_PER_YEAR);
}
